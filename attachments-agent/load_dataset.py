    ...
    for example in dataset:
        ...
        if is_text_only and evidence_sources:
            ...
            clean_example = {
                ...
                "evidence_pages": (
                    eval(example["evidence_pages"]) if example["evidence_pages"] else []
                ),
...
def download_all_documents(data_dir: Path) -> bool:
    ...
    try:
        ...
        if downloaded_docs_dir.exists():
            ...
            try:
                if (
                    downloaded_docs_dir.name == "documents"
                    and downloaded_docs_dir != data_dir
                ):
                    import shutil
...
def create_document_info(
    text_only_examples: List[Dict[str, Any]], all_doc_ids: List[str], data_dir: Path
):
...
def main():
    ...
    print(
        f"Found {len(text_only_examples)} text-only examples from {len(all_doc_ids)} total documents"
    )

    # Check if we need to download documents
    availability = check_available_documents(all_doc_ids, data_dir)
    missing_docs = [
        doc_id for doc_id, available in availability.items() if not available
    ]
    ...
    if missing_docs:
        ...
        if not download_success:
            print(
                "Warning: Document download failed, evaluation will be limited to available documents"
            )
    else:
    ...
    print(
        f"  Documents with text-only examples: {doc_info['summary']['documents_with_text_only_examples']}"
    )
    print(
        f"  Documents available locally: {doc_info['summary']['documents_available_locally']}"
    )
    ...
    print(
        f"  text_only_eval_examples.json - {len(text_only_examples)} filtered examples"
    )
    ...
    missing_docs = [
        doc_id for doc_id in all_doc_ids if not (data_dir / doc_id).exists()
    ]
    if missing_docs:
        ...
        if missing_text_only:
            print(
                f"     Priority (needed for text-only evaluation): {len(missing_text_only)} documents"
            )
