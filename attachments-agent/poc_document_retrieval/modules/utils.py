...
class BM25Retrieval(RetrievalBase):
    ...
    def retrieve(self, query: str, documents: List[str], top_k: int = 10) -> List[str]:
        ...
        avg_doc_len = sum(len(self._tokenize(doc)) for doc in documents) / len(
            documents
        )
        ...
        try:
            ...
            if "gemma" in model_name.lower():
                ...
                self.model = SentenceTransformer(
                    model_name, model_kwargs={"torch_dtype": torch.float16}
                )
            self.np = np
            self.cosine_similarity = cosine_similarity
        except ImportError:
            raise ImportError(
                "sentence-transformers and scikit-learn required for SentenceTransformerRetrieval"
            )
...
class InstructSentenceTransformerRetrieval(RetrievalBase):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "sentence-transformers and scikit-learn required for InstructSentenceTransformerRetrieval"
            )
...
class QwenReranker(RerankerBase):
    ...
    def __init__(
        ...
        instruction: str = "Determine if this document is relevant to the query.",
    ):
        try:
            ...
            self.true_token_id = self.tokenizer.encode("Yes", add_special_tokens=False)[
                0
            ]
            self.false_token_id = self.tokenizer.encode("No", add_special_tokens=False)[
                0
            ]
    ...
    def rerank(self, query: str, documents: List[str], top_k: int = 5) -> List[str]:
        ...
        for pair in pairs:
            ...
            inputs = self.tokenizer(
                pair, return_tensors="pt", truncation=True, max_length=2048
            )
            ...
            with torch.no_grad():
                ...
                true_prob = torch.softmax(
                    logits[[self.true_token_id, self.false_token_id]], dim=0
                )[0]
...
def load_retrieval(retrieval_method: str, **kwargs) -> RetrievalBase:
    ...
    if retrieval_method == "bm25":
        ...
        return InstructSentenceTransformerRetrieval(
            model_name=model_name, instruction=instruction
        )
    else:
...
def load_reranker(reranker_method: str, **kwargs) -> Optional[RerankerBase]:
    ...
    if reranker_method is None:
        ...
        instruction = kwargs.get(
            "instruction", "Determine if this document is relevant to the query."
        )
