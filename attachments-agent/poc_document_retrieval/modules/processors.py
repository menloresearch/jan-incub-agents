...
class PyPDF2Processor(ProcessorBase):
    ...
    def file_to_text(self, file: Path) -> str:
        ...
        if file.suffix.lower() != ".pdf":
            raise ValueError(
                f"PyPDF2Processor only supports PDF files, got: {file.suffix}"
            )
...
class MarkItDownProcessor(ProcessorBase):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            raise ImportError(
                "MarkItDown library not installed. Install with: pip install markitdown"
            )
...
class MarkerProcessor(ProcessorBase):
    ...
    def file_to_text(self, file: Path) -> str:
        ...
        if file.suffix.lower() != ".pdf":
            raise ValueError(
                f"MarkerProcessor only supports PDF files, got: {file.suffix}"
            )
